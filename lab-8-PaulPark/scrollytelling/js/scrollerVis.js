class ScrollerVis {
  ...
  initVis() {
    ...
    vis.xScale = d3.scaleLinear()
        ...
        .domain([0, d3.max(vis.data, d => d.time)]);
  ...
  step1() {
    ...
    vis.rect.transition()
      .attr('fill', d => d.difficulty === 'Easy' ? '#51855f' : '#e0e0e0') // highlighted vs default  
      }
  ...
  step2() {
    ...
    vis.rect.transition()
      .attr('fill', d => d.difficulty === 'Difficult' ? '#51855f' : '#e0e0e0') // highlighted vs default  
      }
  ...
  step3() {
    ...
    vis.rect.transition().duration(500)
        .attr('opacity', 1)
        .attr('fill', d => d.name === vis.dataLongestAssignment.name ? '#d7191c' : '#e0e0e0')       
        .attr('width', d => vis.config.cellWidth)
        .attr('height', d => vis.config.cellHeight)
        .attr('x', (d, i) => i % vis.config.columns * (vis.config.cellWidth + vis.config.cellSpacing))
        .attr('y', (d, i) => Math.floor(i / vis.config.columns) % vis.config.rows * (vis.config.cellHeight + vis.config.cellSpacing));
